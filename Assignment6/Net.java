...
public class Net {
  ...
  static double errorCriterion = .001; // error threshold for training to stop
  ...
  public Net(int[][] patterns, int[][] targets) {
    ...
    // node.
    // The bias node will always be active, and have no incoming connections.
    Bias = new Unit();
    Bias.activation = 1.0;

    // creates all units and links them
    inUnit1 = new Unit();
    inUnit2 = new Unit();
    outUnit = new Unit();
  ...
  public double computeError() {
    return Math.pow(this.outUnit.error, 2);
  }
  ...
  public void train() { 
    ...
    while (curEpochs < numEpochs && computeError() >= errorCriterion){

      // for each training example, feed and update the net, log it
      for (int i = 0; i < 4; i++){
        if (curEpochs < 10){
          System.out.println("\nEpochs: " + curEpochs);
          logActivationCalculation();   // logs activation calc
          logWeightUpdates();
        }
        feedforward(trainingData[i]);   // feeds forward
        outUnit.computeError(trainingTargets[i][0]);    // computes error
        outUnit.computeWeightChange();    // computes the weight change
        outUnit.updateWeights();    // changes the weights
      }

      curEpochs++;  // move to next epoch
    }
